...
# parameters for adding images
#imageDir="/home/stomics_data/liminData/stereopy_demo/formatConvertion/seurat/imgDir/"
#binsize, lowres, hires


args <- arg_parser("Converting h5ad file(.h5ad) to RDS.")
args <- add_argument(args, "--infile", help = "input .h5ad file from stereopy format convertion")
#args <- add_argument(args, "--outfile", help = "output RDS file")
args <- add_argument(args, "--bin", help = "bin size, default is 200")
args <- add_argument(args, "--image_dir", help = "The path of the tissue_lowres_image.png")
...
outfile=str_replace(basename(infile), "\\.h5ad","\\.RDS")
...
Convert(infile, dest = paste0(imageDir,"/",filename,'.h5seurat'),assay = assay, overwrite = TRUE)

#f <- H5File$new(paste0("./",filename,'.h5seurat'), "r+")
f <- H5File$new(paste0(imageDir,"/",filename,'.h5seurat'), "r+")
...
h5_file <- paste0(imageDir,'/',filename,'.h5seurat')
...
#saveRDS(object, outfile) # rds before adding real image


print("add image to rds!")
...
SpatialFeaturePlot(obj, features = "nFeature_Spatial", stroke = 0.25,pt.size.factor = 1, image.alpha = 1, alpha = 0.2)
dev.off()

file.remove(paste0(imageDir, "/",file_path_sans_ext(outfile),".h5seurat"))
print("Finished RDS.")
